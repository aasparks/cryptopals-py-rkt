...
; encrypt: encrypts the pt under the key using MT19937 as a stream cipher
...
; decrypt: decrypts the MT19937 cipher
...
; get-seed: get the key from a known plaintext 
(define (get-seed)
...
; reset-token: generate a reset token using MT19937 seeded with the current time
(define (reset-token)
  (define mt (new MT19937% [seed (current-milliseconds)]))
  (apply bytes
         (for/list ([i (in-range 6)]) (bitwise-and (send mt generate-number) #xFF))))
...
; check-token: determines if a token was created by seeding MT19937
